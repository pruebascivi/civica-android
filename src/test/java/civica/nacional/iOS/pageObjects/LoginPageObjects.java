...
public class LoginPageObjects extends PageObject {
	...
	private String DocumentoTi = "//XCUIElementTypeStaticText[@name = 'Tarjeta de Identidad']";
	private String DocumentoCc = "//*[@name = 'Selección Cédula de Ciudadanía']";
	private String DocumentoCe = "//XCUIElementTypeStaticText[@name = 'Cédula de Extranjería']";
	...
	private String txtMiDaviPlataXpath = "//XCUIElementTypeStaticText[@name='Mi DaviPlata']";
	...
	private String btnMenuHamburguesa = "//XCUIElementTypeButton[@name='Menú DaviPlata botón']";
	private String btnUsarPlata = "//XCUIElementTypeApplication[@name='DaviPlata']/XCUIElementTypeWindow[1]/XCUIElementTypeOther[3]/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther[3]/XCUIElementTypeOther[2]/XCUIElementTypeButton";
...
//	private String btnContinuarActivar = "//android.widget.LinearLayout[2]/android.widget.Button[2]";
	...
	private String mensajeCaracter = "(//XCUIElementTypeStaticText[contains(@name,'solo')])";
	...
	public void capturarSaldo() {
		try {
			...
			if (base.saldo == null) {
				System.out.println("saldo: "+ subSaldo);
				base.saldo = new BigDecimal(subSaldo);
				System.out.println("base saldo captura: " + base.saldo);
				String saldo1 = subSaldo;
				float saldoDavi1 = Float.parseFloat(saldo1);
				saldos.add(saldoDavi1);
			} else {
				System.out.println("saldo final : " + subSaldo);
...
//				pageNLatinia.darClickFinalizarSesion();
	...
	public boolean verificoSiEstaRegistrado() {
		...
		try {
			// MobileElement labelActualizarCorreo = (MobileElement) wait
			// .until(ExpectedConditions.elementToBeClickable(By.xpath(this.labelActualizarCorreo)));
	...
	public void validoIngresoALaAPP(String mensajeActual) {
		assertEquals("Mi DaviPlata", mensajeActual);
	}
...
//*************************************************Validacion Mensajes y notificaciones**************************************************	
	...
	public void btnCompras() {
		try {
			...
			if(!(contador==5)) {
				...
				fail("No se encontró botón 'Tienda virtual' de mensajes del cliente en la campana de notificaciones, debido a: " + e.getMessage());
			}
		}finally {contador=0;}
	...
	public void ingresoOTPRegistro(String OTP) {
		try {
			...
			//for(int i = 1; i <= 6; i++) {
			//	driver.findElement(By.xpath("//XCUIElementTypeTextField['"+ i +"']")).sendKeys(otpDividida[i - 1]);
			//}
	...
	public void seleccionarTipoDocumentoClearInput(String tipoDocumento) {
		try {
			utilidad.esperaMiliseg(500);
			contador++;
			switch (tipoDocumento) {
			case "TI":
				base.driver.findElement(By.xpath(this.tipoDocumento)).click();
				base.driver.findElement(By.xpath(this.DocumentoTi)).click();
				utilidad.tomaEvidencia("Selecciono tipo de documento");
				break;
			case "Cédula de ciudadanía":
				base.driver.findElement(By.xpath(this.tipoDocumento)).click();
				base.driver.findElement(By.xpath(this.DocumentoCc)).click();
				utilidad.tomaEvidencia("Selecciono tipo de documento");
				break;
			case "CC":
				base.driver.findElement(By.xpath(this.tipoDocumento)).click();
			...
			case "CE":
				base.driver.findElement(By.xpath(this.tipoDocumento)).click();
				base.driver.findElement(By.xpath(this.DocumentoCe)).click();
				utilidad.tomaEvidencia("Selecciono tipo de documento");
				break;
			case "PEP":
				base.driver.findElement(By.xpath(this.tipoDocumento)).click();
			...
			if (!(contador == 5)) {
				Utilidades.esperaMiliseg(500);
				seleccionarTipoDocumentoClearInput(tipoDocumento);
			} else {
				fail("No se pudo seleccionar tipo de documento en el login debido a: " + e.getMessage());
			}
		} finally {
			contador = 0;
		}
	...
	public void validarMensajeCaracteres() {
		try {
			contador++;
			String mensajeCaracter = driver.findElement(By.xpath(this.mensajeCaracter)).getText();
			assertThat(mensajeCaracter, containsString("Su identificacion/celular debe tener"));
		} catch (Exception e) {
			if (!(contador == 5)) {
				utilidad.esperaMiliseg(2000);
				validarMensajeCaracteres();
			} else {
				fail("No se encontró mensaje de caracteres en logeo de la app debido a: " + e.getMessage());
			}
		} finally {
			contador = 0;
		}
	...
	public void validarLongitudCaracteres(String usuario) {
		try {
			...
			MobileElement carcateresLongitud = (MobileElement) wait
					.until(ExpectedConditions.visibilityOfElementLocated(By.id(this.edtLoginId)));
	...
	public void darClickMenuHamburguesa() {
		MobileElement btnMenuHamburguesa = (MobileElement) wait
				.until(ExpectedConditions.elementToBeClickable(By.xpath(this.btnMenuHamburguesa)));
		btnMenuHamburguesa.click();
	}

	public void pulsarBtnMasServicios() {
		MobileElement btnMasServicios = (MobileElement) wait
				.until(ExpectedConditions.elementToBeClickable(By.xpath(this.btnMasServicios)));
		btnMasServicios.click();
		utilidad.esperaMiliseg(800);
		utilidad.tomaEvidencia("click botón mas servicios");
	}
	...
	public void validacionCaracterEspecial() {
		try {
			contador++;
			int j = 1;
			System.out.println("entre a validar la clave del caracter especial");
			for (int i = 0; i < 4; i++) {
				MobileElement edtContrasena = (MobileElement) wait
						.until(ExpectedConditions.elementToBeClickable(By.name("otpTextField_" + j + "")));
	...
	public void validarInicioSesionPersona() {
        boolean lblIngresaPresente = validarPerfilNegocioPresente();
        if(lblIngresaPresente) {
        	negocioPO.darClicBotonMiDaviplata();
	...
	public boolean validarPerfilNegocioPresente() {
        ...
        while((!aux) && (cont<7)) {
            try {
            	base.driver.findElement(By.xpath(this.homePerfilNegocio));aux=true;
            }catch (Exception e) {
            	aux=false;
            }
            utilidad.esperaMiliseg(500);
