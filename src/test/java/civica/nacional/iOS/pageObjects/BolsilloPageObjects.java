...
public class BolsilloPageObjects extends PageObject {
	...
	private String lblSaldoTotalBolsillos = "com.davivienda.daviplataapp.lab:id/pocketBalance";
	private String btnActualizarSaldoBolsillos = "com.davivienda.daviplataapp.lab:id/refresh_pocket";
	private String btnCrearBolsillo = "	//XCUIElementTypeButton[@name='Abrir bolsillo']";
	private String btnCrearBolsillo2 = "Abrir bolsillo";
	private String btnBolsillo = "Bolsillo";
	private String btnBolsillos = "(//XCUIElementTypeButton)[11]";
	private String btnBolsilloHome = "//XCUIElementTypeButton[@name='Abrir bolsillo' or @name='Abrir Bolsillo' or @name='vo.daviplata.home.bolsilloDisponible' or @name='img home saldodisponible']";
	...
	private String txtNombreBolsillo = "(//XCUIElementTypeOther[1])[9]";
	private String txtCrearNombrebolsillo = "//XCUIElementTypeOther[@name='Campo editable nombre del bolsillo: ']";
	private String txtMontoInicialBolsillo = "Campo editable Con cuánta plata abrira el bolsillo?: pesos";
	private String txtMontoTotalBolsillo = "Campo editable Cuanta plata quiere ahorrar?: pesos";
	private String btnListaPeriodo = "(//XCUIElementTypeOther[3])[2]";
	...
	// private String btnPeriodo =
	// "com.davivienda.daviplataapp.lab:id/bolsillo_btn_frecuencia_ahorro";
	...
	// private String btnQuincenal = "//XCUIElementTypePickerWheel[@value = 'Cada
	// Semana']";
	...
	public void pulsarBtnCrearBolsillos() {
		try {
			...
			System.out.println("Estoy desde Bolsillo Page Objects");
			wait.until(ExpectedConditions.elementToBeClickable(By.xpath(this.btnBolsilloHome)));
			driver.findElement(By.xpath(this.btnBolsilloHome)).click();
	...
	public void ingresarNombreBolsillo() {
		try {
            ...
            touchAction.tap(new PointOption().withCoordinates(128, 230)).perform();
			String nombre = this.objFaker.name().firstName();
			
			for(int i = 0; i<nombre.length(); i++) {
				String letra = String.valueOf(nombre.charAt(i)).toLowerCase();
				String teclas = "//XCUIElementTypeKey[@name='" + letra + "']";
			...
			if (!(contador == 5)) {
				Utilidades.esperaMiliseg(2000);
				ingresarNombreBolsillo();
			} else {
				fail("No se pudo ingresar el nombre al bolsillo debido a: " + e.getMessage());
			}
		} finally {
			contador = 0;
		}
	...
	public void crearBolsilloNombre() {
		try {
			contador++;
			...
			base.nombreBolsillo = nombre.toUpperCase();
	...
	public void ingresarMontoInicialBolsilloFondosInsuficientes(String monto) {
		try {
			...
			utilidad.esperaMiliseg(2000);
			TouchAction touchAction=new TouchAction(driver);
            touchAction.tap(new PointOption().withCoordinates(110, 191)).perform();
	...
	public void ingresarMontoTotalBolsillo(String monto) {
		...
		try {
			...
			utilidad.esperaMiliseg(2000);
			TouchAction touchAction=new TouchAction(driver);
            touchAction.tap(new PointOption().withCoordinates(110, 270)).perform();
	...
	public void setSaldoBolsillo() {
		try {
			...
			String saldoBolsilloTotal = saldoBolsillo.replace("Consulta", "").replace("Nombre", "")
					.replace("del", "").replace("Bolsillo", "").replace(".", "").replace("Saldo", "").replace(":", "")
					.replace("pesos", "").replace("bolsillo", "").replace("total", "").replace(",", "")
					.replace(" ", "");
	...
	public void ingresoPeriodo() {
		try {
			contador++;
			
			TouchAction touchAction=new TouchAction(driver);
            touchAction.tap(new PointOption().withCoordinates(131, 430)).perform();
            touchAction.tap(new PointOption().withCoordinates(120, 355)).perform();

			System.out.println("di click en la lista de periodos");
