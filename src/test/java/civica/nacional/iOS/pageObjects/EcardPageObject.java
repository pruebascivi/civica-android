...
//	private String btnRecargar = "//*[contains(text(),'RecargarTarjeta')]";
	...
	private String txtRestringida = "(//XCUIElementTypeStaticText[contains(@name,'bloqueada')or contains(@name,'bloqueado') or contains(@name,'Bloqueado') or contains(@name,'Bloqueada')])";
	...
	public void doyClickEnContinuar() {
		try {
			...
			if(!(contador==20)) {Utilidades.esperaMiliseg(2000); doyClickEnContinuar();
	...
	public void aceptoTerminos() {
		wait.until(ExpectedConditions.elementToBeClickable(By.xpath(this.btnTerminos)));
	...
	public void validoRecargaRestringida() {
		try {
			contador++;
			wait.until(ExpectedConditions.elementToBeClickable(By.xpath(this.txtRestringida)));
			String txtRestringida = driver.findElement(By.xpath(this.txtRestringida)).getText();
			boolean apareceMensaje = false;
			if(txtRestringida.toLowerCase().contains("bloqueado") || txtRestringida.toLowerCase().contains("bloqueada")) 
				apareceMensaje = true;
			assertTrue(apareceMensaje);
		}catch(Exception e) {
			if(!(contador==5)) {
				utilidad.esperaMiliseg(500);
				validoRecargaRestringida();
			}else {
				fail("No se pudo encontrar mensaje de tarjeta bloqueada debido a: " + e.getMessage());
			}
		}finally {contador=0;}
...
//		subSaldo = saldoFinalTarjeta.subtract(saldoInicialTarjeta);
	...
	public void pulsarBtnMovimientos() {
		wait.until(ExpectedConditions.elementToBeClickable(By.xpath(this.btnMovimientos)));
	...
	public void pulsarAceptarPopUpBloquear() {
		try {
			...
			if(!(contador==5)) {
				Utilidades.esperaMiliseg(contador);
	...
	public void obtenerMovimientosTarjeta() {
		...
		do {
			try {
				...
				wait.until(ExpectedConditions.elementToBeClickable(By.xpath(xpathFecha)));
    ...
    public void montoTarjetaParaRecargar(String monto) {
    	try {
    	    ...
    	    case "50000":
    	     ...
    	     break;
    	    case "100000":
    	    	wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(this.checkboxDiezMil)));
        		driver.findElement(By.xpath(this.checkboxDiezMil)).click();
    	     break;    	
    	    case "300000" :
    	    	wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(this.checkboxTrecientosMil)));
        		driver.findElement(By.xpath(this.checkboxTrecientosMil)).click();
    	     break;
    	    case "500000" :
    	    	wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(this.checkboxQuinientosMil)));
    	    ...
    	    default: {
        		...
        		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(this.btnContinuar)));
        		driver.findElement(By.xpath(this.btnContinuar)).click();
    	    }
