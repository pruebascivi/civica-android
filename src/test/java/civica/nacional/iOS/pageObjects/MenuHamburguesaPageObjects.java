...
public class MenuHamburguesaPageObjects extends PageObject {
	...
	private String btnVerMovimientos = "(//XCUIElementTypeButton[@name='Button'])[2]";
	...
	public void darClickMasServicios() {
		try {
			...
			if(!(contador==10)) {
				...
				fail("No se encontró botón 'Más Servicios', debido a: " +e.getMessage());
			}
		}finally {contador=0;}
	...
	public void darClickCambiarClaveCorreo() {
		try {
			contador++;
			wait.until(ExpectedConditions.elementToBeClickable(By.xpath(this.btnCambiarClaveCorreo)));
			driver.findElement(By.xpath(this.btnCambiarClaveCorreo)).click();
		}catch(Exception e) {
			if(!(contador==10)) {
				Utilidades.esperaMiliseg(500);
				darClickCambiarClaveCorreo();
			}else {
				fail("No se encontró botón 'Cambiar Clave', debido a: " +e.getMessage());
			}
		}finally {contador=0;}
